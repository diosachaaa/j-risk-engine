...
export function buildDashboardSummary(rowsInput = []) {
  ...
  const summary = rows.reduce(
    (result, row) => {
      ...
      if (
        !result.latestRow ||
        toTimestamp(row.updatedAtRaw) >
          toTimestamp(result.latestRow.updatedAtRaw)
      ) {
...
function getDominantLevel(summary) {
  ...
  return (
    levels.sort((left, right) => right.value - left.value)[0]?.level ?? 'low'
  );
}
...
export function buildManagementInsightsData(
  ...
  const locale = normalizeLocale(options.locale);
  const summary = options.summaryOverride || buildDashboardSummary(rowsInput);
  ...
  const riskIncrease =
    normalizedTrendData.length >= 2 &&
    (normalizedTrendData[0]?.value ?? 0) !== 0
      ? Math.round(
