...
function buildFallbackHistory(currentRiskScore, locale = DEFAULT_LOCALE) {
  ...
  return offsets.map((offset) => {
    ...
    const derivedScore = clampRiskScore(
      currentRiskScore - Math.max(0, Math.abs(offset) - 2),
    );
...
function buildRiskHistoryAnalysis(riskHistory = [], locale = DEFAULT_LOCALE) {
  ...
  const values = riskHistory.map((item) =>
    toNumber(item.value ?? item.score, 0),
  );
...
function buildTechnicalAnalysis(viewModelSource, locale = DEFAULT_LOCALE) {
  const {
    currentRiskScore,
    riskLevel,
    riskComponents,
  ...
  return points.map((point, index) => {
    ...
    return {
      ...
      detail: normalizeString(
        pickFirst(
          point,
          ['detail', 'description', 'summary', 'note'],
          fallbackDetail,
        ),
        fallbackDetail,
      ),
...
export function buildAssetDetailViewModel({
  ...
  const currentRiskScore =
    mappedScore.currentRiskScore !== undefined
      ? mappedScore.currentRiskScore
      : 0;

  const riskLevel =
    mappedScore.riskLevel ||
    getRiskLevelLabel(currentRiskScore, normalizedLocale);
  ...
  return {
    ...
    riskHistory,
    riskHistoryAnalysis: buildRiskHistoryAnalysis(
      riskHistory,
      normalizedLocale,
    ),
    vulnerabilities,
    ...
    technicalAnalysis: buildTechnicalAnalysis(
      viewModelSource,
      normalizedLocale,
    ),
